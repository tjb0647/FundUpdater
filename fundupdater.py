...
def grabfunds(fundlist):
    if len(fundlist) > 0:
        ...
        for fund in fundlist:
            ...
            df1 = grabinfo(
                Fund(fund, type=fundlist[fund]), driver=driver).sectors
            df = df.append(df1)
            i = i + 1
        driver.quit()
...
def grabinfo(Fund, type=None, driver=initiate_browser()):
    ...
    time.sleep(2)
    sectors = driver.find_element(
        By.CLASS_NAME, "sal-sector-exposure__sector-table").text
    sectors_footer = driver.find_element(
        By.CLASS_NAME, "sal-sector-exposure__sector-footer").text
    ...
    d.update(dstyles)
    d.update(dsectors)
    d['Date'] = (re.search(r'of (...+)', sectors_footer.split("|")
                 [0]).groups()[0]).strip()
...
def append_history(funds):
    ...
    funds.sort_values(by=['ticker', 'Date'], ascending=[
                      True, False]).to_csv(HISTORY)
    pass
